...
class LocationView(APIView):
    ...
    def get(self, request):
        user = request.user
        return Response(
            LocationSerializer(user.location).data, status=status.HTTP_200_OK
        )

    @swagger_auto_schema(
        request_body=UserLocationValidator, responses={200: UserLocationSerializer}
    )
...
class NeighborhoodView(APIView):
    ...
    def get(self, request, location_code):
        ...
        return Response(
            NeighborhoodSerializer(location.neighborhoods, many=True).data,
