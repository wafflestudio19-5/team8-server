...
class PostLocationTestCase(TestCase):
    ...
    def setUp(cls):
        ...
        cls.user_token = "JWT " + jwt_token_of(
        ...
        LocationNeighborhood.objects.create(
            location=cls.location1, neighborhood=cls.location2
        )
    ...
    def test_post_location_wrong_information(self):
        ...
        response = self.client.post(
            "/api/v1/location/", data=data, HTTP_AUTHORIZATION=self.user_token
        )
...
class GetLocationTestCase(TestCase):
    ...
    def test_get_location_sucess(self):
        ...
        response = self.client.get(
            "/api/v1/location/", HTTP_AUTHORIZATION=self.user_token
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class GetNeighborhoodTestCase(TestCase):
    ...
    def setUp(cls):
        ...
        LocationNeighborhood.objects.create(
            location=cls.location1, neighborhood=cls.location3
        )
    ...
    def test_get_neighborhood_sucess(self):
        ...
        response = self.client.get(
            "/api/v1/location/neighborhood/", HTTP_AUTHORIZATION=self.user_token
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        self.assertEqual(
            {res_data[0]["code"], res_data[1]["code"]}, {"1111011100", "1111010100"}
        )
