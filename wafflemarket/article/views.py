...
class ArticleViewSet(viewsets.GenericViewSet):
    ...
    def create(self, request):
        ...
        for i in range(1, image_count + 1):
            ...
            if request.FILES.get(field_name) is None:
                return Response({"업로드 형식이 올바르지 않습니다."}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def comment(self, request, pk):
        ...
        if self.request.method == "POST":
            ...
            return Response(
                CommentSerializer(comments, many=True, context={"user": request.user}).data, status=status.HTTP_201_CREATED
            )

        elif self.request.method == "GET":
            comments = Comment.objects.filter(article=article)
            return Response(
                CommentSerializer(comments, many=True, context={"user": request.user}).data, status=status.HTTP_200_OK
            )
