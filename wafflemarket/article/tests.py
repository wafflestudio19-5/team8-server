...
class PostCommentTestCase(TestCase):
    ...
    def test_post_comment_wrong_information(self):
        ...
        # invalid content
        response = self.client.post('/api/v1/article/%s/comment/'%pk, data={'content' : '  '}, HTTP_AUTHORIZATION=self.user2_token)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class DeleteCommentTestCase(TestCase):
    ...
    def test_delete_comment_not_commenter(self):
        ...
        c_id = str(self.comment1.id)
        # wrong commenter
        response = self.client.delete('/api/v1/article/%s/comment/%s/'%(a_id, c_id), data={}, HTTP_AUTHORIZATION=self.user1_token)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_delete_comment_wrong_article_id(self):
        ...
        response = self.client.delete('/api/v1/article/%s/comment/%s/'%(a_id, c_id), data={}, HTTP_AUTHORIZATION=self.user2_token)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
...
class PostReplyTestCase(TestCase):
    ...
    def test_post_reply_wrong_article_id(self):
        ...
        data = self.post_data.copy()
        response = self.client.post('/api/v1/article/%s/comment/%s/'%(a_id, c_id), data=data, HTTP_AUTHORIZATION=self.user1_token)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_post_reply_wrong_information(self):
        ...
        # invalid content
        response = self.client.post('/api/v1/article/%s/comment/%s/'%(a_id, c_id), data={'content' : '  '}, HTTP_AUTHORIZATION=self.user1_token)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
