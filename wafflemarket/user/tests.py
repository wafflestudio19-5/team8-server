...
class PutUserCategoryTestCase(TestCase):
    ...
    def test_put_category_wrong_information(self):
        ...
        response = self.client.put('/api/v1/user/category/', data=data, content_type='application/json', HTTP_AUTHORIZATION=self.user_token)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_put_category_success(self):
        ...
        response = self.client.put('/api/v1/user/category/', data={'category':'가구/인테리어', 'enabled':'FALSE'}, content_type='application/json', HTTP_AUTHORIZATION=self.user_token)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertNotIn("디지털기기", response.json()['category'])
...
class GetUserCategoryTestCase(TestCase):
    ...
    def test_get_category_success(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()['category'], ['디지털기기', '가구/인테리어', '여성의류', '게임/취미', '삽니다', '생활가전', '여성잡화', '남성패션/잡화', '기타 중고물품'])